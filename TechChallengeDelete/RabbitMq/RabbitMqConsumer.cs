...
using System.Text;
using TechChallengeDelete.RabbitMq;

namespace TechchallengeAdd.RabbitMq
{
        ...
        protected override Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            consumer.Received += async (model, ea) =>
            {
                ...
                using (var scope = _serviceProvider.CreateScope())
                {
                    ...
                    if (contact != null)
                    {
                        await contactApplication.DeleteAsync(contact.Id);
                        Console.WriteLine($"Contato  Deteletado: {contact}");
                    }
                }
            };

            _channel.BasicConsume(queue: "contact_deleted", autoAck: true, consumer: consumer);
